...
**Task** \* Duplicate the last line in this code chunk and modify it by replacing the single `>` with `>>`. What happens when you rerun it?
...
#head -n 25 /local-scratch/course_files/MBB243/Sneddon_genotypes.txt >> ~/MBB243/genotype_head.txt
#cat ~/MBB243/genotype_head.txt
# > : similar to save As option. Create and save a new file or if it exists overwrite the current file with the new data
...
#cut -f 4 ~/MBB243/genotype_head.txt > ~/MBB243/genotype_cut.txt
#cut : taking the stated coloumn number
#sort ~/MBB243/genotype_cut.txt > ~/MBB243/genotype_sort.txt
#sort: sort by 1st stated coloumn
#uniq ~/MBB243/genotype_sort.txt > ~/MBB243/genotype_uniq.txt
#uniq: take out uniq values
#cat ~/MBB243/genotype_uniq.txt
#cat: just state the given values
...
# > save as new text into the file and overwrite all existing data
echo "DATA=/local-scratch/course_files/MBB243/" >> ~/.bash_profile
# >> add to the file created above "ADD" not overwrite
cat ~/.bash_profile
```
...
**Task** Run this command in your terminal and make sure it completes successfully. Test it by simply entering `samtools` in your terminal (followed by <ENTER>).
...
```{bash}
seq=$(samtools faidx /local-scratch/course_files/MBB243/hg38.fa chr14:65102304-65102339 | grep -v \> )
echo $seq
```

## Lab Completion Checklist

**Task** At the end of every lab, replace TODO with DONE to the left of each task to indicate you've completed it
